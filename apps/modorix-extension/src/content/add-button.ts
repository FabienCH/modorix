...
export async function addButtonToCard(linkElement: HTMLAnchorElement) {
  ...
  addButton(cardElement, linkElement);
  addButtonIfCardChanges(cardElement, linkElement);
}

function addButton(cardElement: HTMLElement, linkElement: HTMLAnchorElement) {
  const modorixBlockButton = cardElement.querySelector("#modorix-block-button");
  const cardLink =
    cardElement.children[0]?.children[0]?.children[1]?.querySelector("a");
...
function addButtonIfCardChanges(
  ...
  const lookupInterval = setInterval(async () => {
    ...
    if (newCardElement && newCardElement !== cardElement) {
      ...
      addButton(newCardElement, linkElement);
    }
  }, 100);
  setTimeout(() => {
    clearInterval(lookupInterval);
